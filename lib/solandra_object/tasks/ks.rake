namespace :ks do
  ...
  task :create => :configure do
    SolandraObject::Tasks::Keyspace.new.create @config['keyspace'], @config
  ...
  task :drop => :configure do
    SolandraObject::Tasks::Keyspace.new.drop @config['keyspace']
  ...
  task :migrate => :configure do
    ...
    SolandraObject::Schema::Migrator.migrate SolandraObject::Schema::Migrator.migrations_path, version
    schema_dump
  end
  ...
  task :rollback => :set_keyspace do
    ...
    SolandraObject::Schema::Migrator.rollback SolandraObject::Schema::Migrator.migrations_path, step
    schema_dump
  end
  ...
  task :forward => :set_keyspace do
    step = ENV['STEP'] ? ENV['STEP'].to_i : 1
    SolandraObject::Schema::Migrator.forward SolandraObject::Schema::Migrator.migrations_path, step
    schema_dump
  end

  namespace :schema do
    desc 'Create ks/schema.json file that can be portably used against any Cassandra instance supported by SolandraObject'
  ...
  task :version => :set_keyspace do
    version = SolandraObject::Schema::Migrator.current_version
  ...
  def schema_load(env = Rails.env)
    ...
    File.open "#{Rails.root}/ks/schema.json", 'r' do |file|
      ...
      ks.schema_load SolandraObject::Tasks::Keyspace.parse(hash)
    end
  end

  def set_keyspace(env = Rails.env)
    config = @configs[env.to_s || 'development']
    ks = SolandraObject::Tasks::Keyspace.new
