  ...
  class HasAndBelongsToMany < CollectionAssociation #:nodoc:
    ...
    private
      ...
      def check_for_join_column_family
        unless SolandraObject::Base.connection.column_families.has_key?("many_to_many_joins")
          cf = Cassandra::ColumnFamily.new
          cf.name = "many_to_many_joins"
          cf.keyspace = SolandraObject::Base.connection.keyspace
          cf.comparator_type = 'BytesType'
          cf.column_type = 'Standard'
          SolandraObject::Base.connection.add_column_family(cf)
        end
      end
  end
end