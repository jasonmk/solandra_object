...
# 
      # def find_by_id(keys)
        # find(keys)
      # rescue SolandraObject::RecordNotFound
        # nil
      # end
# 
      # def all(options = {})
        # limit = options[:limit] || 100
        # results = ActiveSupport::Notifications.instrument("get_range.solandra_object", :column_family => column_family, :key_count => limit) do
...
# 
      # def multi_get(keys, options={})
        # attribute_results = ActiveSupport::Notifications.instrument("multi_get.solandra_object", :column_family => column_family, :keys => keys) do
